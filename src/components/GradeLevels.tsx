...
const GradeLevels = () => {
  const [selected, setSelected] = useState<
    Set<"grade9" | "grade10" | "grade11" | "grade12">
  >(new Set());
    ...
    <div className="flex flex-col gap-12 z-1">
      ...
      <div className="grid grid-cols-2 gap-4 justify-items-center">
        {(["grade9", "grade10", "grade11", "grade12"] as const).map((grade) => (
          ...
          >
            <div
              className={`flex items-center justify-center w-7 h-7 border-2 transition-all duration-300 rounded-md ${selected.has(grade) ? "bg-secondary border-secondary-50" : "border-text-50 group-hover:border-secondary-50"}`}
            >
              {selected.has(grade) && (
